...
export const ProductScreenComponents = () => {
  ...
  const handleAddToCart = () => {
    ...
    const detalleEnCarrito = usercart?.find((d) => d.id === detalleSeleccionado.id);
    ...
    if (cantidadActual + cantidad > detalleSeleccionado.stock) {
      Swal.fire({
        ...
        text: `No hay suficiente stock para agregar esa cantidad (stock disponible: ${detalleSeleccionado.stock - cantidadActual}).`,
      });
      return;
    }
            ...
            <div className={styles.quantityContainer}>
              ...
              <button
                ...
                onClick={() => {
                  if (
                    detalleSeleccionado &&
                    cantidad < (detalleSeleccionado.stock - (usercart?.find((d) => d.id === detalleSeleccionado.id)?.cantidad || 0))
                  ) {
                    setCantidad((c) => c + 1);
                  }
                }}
                  ...
                  cantidad >=
                    (detalleSeleccionado
                      ? detalleSeleccionado.stock -
                        (usercart?.find((d) => d.id === detalleSeleccionado.id)?.cantidad || 0)
                      : 1)
                }
              >
                +
